...
public partial class index : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            Repeater1.DataSource = verma.Show_Data("SELECT users.*, departments.title as depart_title FROM users INNER JOIN departments ON users.department_id = departments.id where users.industry_id='" + verma.Get_ind()+"'");
            Repeater1.DataBind();
        }
    }

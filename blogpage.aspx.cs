...
public partial class blogpage : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            DisplayRepeter();
            //Repeater1.DataSource = verma.Show_Data("select count(*) as blog_count,services.title as exp1,services.id as depart_id from blogs INNER JOIN services ON blogs.service_id = services.id where blogs.status=1 group by services.title,services.id");
            //Repeater1.DataBind();
            //Repeater2.DataSource = verma.Show_Data("SELECT TOP 10 * FROM blogs where status=1;");
            //Repeater2.DataBind();
    ...
    //{
    //    BlogRepeater.DataSource = verma.Show_Data("select * from blogs where status=1 and service_id='" + e.CommandArgument + "'order by position");
    //    BlogRepeater.DataBind();
    //}

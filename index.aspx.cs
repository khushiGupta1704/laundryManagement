...
public partial class index : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            BlogRepeater1.DataSource = verma.Show_Data("select TOP(4) * from blogs where status=1 order by position");
            BlogRepeater1.DataBind();
            sliderRepeater.DataSource = verma.Show_Data("select * from sliders where status=1 and page_type='Home' order by position");
            ...
            TeamRepeater.DataSource = verma.Show_Data("select * from our_team where status=1 order by position");
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                image_offer_style.ImageUrl = dt.Rows[0]["image"].ToString();
                Offer_image.NavigateUrl = dt.Rows[0]["url"].ToString();
                Offer_image.Style.Add("width", "100%");

...
public partial class index : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            Repeater1.DataSource = verma.Show_Data("SELECT blogs.*, departments.title as depart_title FROM blogs INNER JOIN departments ON blogs.department_id = departments.id where blogs.industry_id='" + verma.Get_ind() + "'");
            Repeater1.DataBind();
        }
    }
    ...
    protected void confirm_delete_Click(object sender, EventArgs e)
    {
        foreach (RepeaterItem r in Repeater1.Items)
        {
            ...
            if (chk.Checked)
            {
                    ...
                    verma.Save_Data("delete from blogs where id='" + lblid.Text + "'");
                    lable_data.Visible = true;
                    lable_data.Attributes.Add("class", "alert alert-success alert-dismissible fade show");
                    lable_msg.Text = "Blog has Been Deleted Successfully!";
    ...
    protected void confirm_delete_2_Click(object sender, CommandEventArgs e)
    {
            ...
            verma.Save_Data("delete from blogs where id='" + e.CommandArgument + "'");
            lable_data.Visible = true;
            lable_data.Attributes.Add("class", "alert alert-success alert-dismissible fade show");
            lable_msg.Text = "Blog has Been Deleted Successfully!";
        ...
        Repeater1.DataSource = verma.Show_Data("SELECT blogs.*, departments.title as depart_title FROM blogs INNER JOIN departments ON blogs.department_id = departments.id where blogs.industry_id='\" + verma.Get_ind() + \"'\"");
        Repeater1.DataBind();
        ClientScript.RegisterStartupScript(this.GetType(), "Popup", "ShowPopup1();", false);
